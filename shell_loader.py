...
class ShellLoader:
    ...
    def _gatekeeper(self, worker: CommandWorker) -> GatekeeperSlot:
        ...
        self._wire_modes(slot, llm_gk, stop_llm, worker)
        return slot

    def _wire_modes(self, slot: GatekeeperSlot, llm_gk: LLMGatekeeper, stop_llm: object, worker: CommandWorker) -> None:
        self._wire_mode(slot, llm_gk, stop_llm, worker, self._kernel.dictation_gate(),
                        self._kernel.set_dictation_callbacks, self._kernel.request_dictation_stop)
        self._wire_mode(slot, llm_gk, stop_llm, worker, self._kernel.coding_gate(),
                        self._kernel.set_coding_callbacks, self._kernel.request_coding_stop)

    def _wire_mode(self, slot: GatekeeperSlot, llm_gk: LLMGatekeeper, stop_llm: object, worker: CommandWorker,
