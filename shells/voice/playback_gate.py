...
class PlaybackGate(Gatekeeper):
    def __init__(
        ...
        self._log = log
        self._completion = GatekeeperCompletion(llm_provider, log)
    ...
    def _interrupt_or_drop(self, utterance: Utterance, speech: str) -> GateDispatch:
        try:
            ...
            self._log.log("ERROR", f"playback gate parse failed: {exc}")
            classification = "ambient"
        return GateDispatch("interrupt") if classification == "interrupt" else GateDispatch("drop")

    def _complete(self, text: str, speech: str) -> str:
        prompt = build_command_gate_prompt("", True, speech)
        return self._completion.complete(prompt, text, "playback_gate", PRIMARY_SCHEMA)
