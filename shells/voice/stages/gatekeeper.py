...
class LLMGatekeeper(Gatekeeper):
    ...
    def evaluate(self, utterance: Utterance, recent: list[Utterance]) -> GateResult:
        ...
        try:
            return _build_result(_unwrap(json.loads(_extract_json(raw))), self._log)
...
def _build_result(data: dict, log: LogPrinter) -> GateResult:
    reason = str(data.get("reason", ""))
    if reason:
        log.log("GATE", reason)
    classification = str(data.get("classification", "ambient"))
    cleaned = str(data.get("cleaned_text", ""))
    return GateResult(classification in ("command", "conversation"), cleaned, 1.0, {"classification": classification})
