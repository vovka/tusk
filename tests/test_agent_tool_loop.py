import types

from tests.kernel_api_support import make_executor, make_registry_tool
from tusk.kernel.schemas.task_plan import TaskPlan
from tusk.kernel.schemas.tool_call import ToolCall
from tusk.kernel.tool_registry import ToolRegistry


def test_executor_stops_after_repeating_same_tool_call() -> None:
    registry = ToolRegistry()
    registry.register(make_registry_tool("gnome.type_text", "typed"))
    calls = iter([_type_call(), _type_call(), _type_call()])
    llm = types.SimpleNamespace(label="agent", complete_tool_call=lambda *args: next(calls))
    result = make_executor(llm, registry=registry).execute("clear the editor", _plan())
    assert "different action" in result.reply.lower()


def test_executor_allows_more_than_ten_steps_before_done() -> None:
    registry = ToolRegistry()
    registry.register(make_registry_tool("gnome.type_text", "typed"))
    llm = types.SimpleNamespace(label="agent", complete_tool_call=_many_steps())
    result = make_executor(llm, registry=registry).execute("long task", _plan())
    assert result.reply == "Finished after many steps."


def _type_call() -> ToolCall:
    return ToolCall("gnome.type_text", {"text": "hi"}, "call-1")


def _many_steps():
    calls = [ToolCall("gnome.type_text", {"text": str(index)}, f"call-{index}") for index in range(1, 12)]
    calls.append(ToolCall("done", {"reply": "Finished after many steps."}, "call-done"))
    replies = iter(calls)
    return lambda *args: next(replies)


def _plan() -> TaskPlan:
    return TaskPlan("execute", "", ["Type text"], ["gnome.type_text"], "type with one tool")
