...
def test_coding_gate_stops_when_llm_marks_metadata_stop() -> None:
    gate = CodingGate(_llm(_stop_json()), _log())
    assert gate.should_stop("okay tusk, stop coding") is True


def test_coding_gate_keeps_instruction_when_llm_declines_stop() -> None:
    gate = CodingGate(_llm(_literal_json()), _log())
    assert gate.should_stop("add a stop button to the page") is False


def test_coding_gate_falls_back_to_plain_completion() -> None:
    gate = CodingGate(_fallback_llm(), _log())
    assert gate.should_stop("please stop coding now") is True
