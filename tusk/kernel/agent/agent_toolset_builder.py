...
class AgentToolsetBuilder:
    ...
    def runtime_names(self, profile: AgentProfile, request: AgentRunRequest) -> set[str]:
        ...
        if "*" in profile.runtime_allowed_tool_names:
            return self._filter_runtime(request.runtime_tool_names, profile, request)
    ...
    def _filter_runtime(self, names: tuple[str, ...], profile: AgentProfile, request: AgentRunRequest) -> set[str]:
        # only the top-level gate fast path (command profile, no parent) may request every visible
        # tool, and only planner-visible ones; a delegated child impersonating "command" stays narrowed
        if "*" in names and profile.profile_id == "command" and not request.parent_call_id:
