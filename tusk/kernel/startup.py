...
def build_kernel(config: Config, log: ColorLogPrinter, llm_registry: LLMRegistry) -> KernelAPI:
    ...
    kernel = build_api(agent, config, log, llm_registry)
    ToolRuntime(tool_registry, llm_registry, adapter_manager, log).register_tools(kernel)
    return kernel


def build_api(agent: MainAgent, config: Config, log: ColorLogPrinter, llm_registry: LLMRegistry) -> KernelAPI:
    backend = AgentBackendFactory(agent, config, log).create()
    return KernelAPI(CommandMode(backend, log), llm_registry, log)
